...
class Upkeep
{
    ...
    function Creator($db,$data)
    {
    	...
    	$sql = mysqli_prepare($db, "INSERT INTO upkeep(id,company,time,unit,cost,date) VALUES (?,?,?,?,?,?)");
		mysqli_stmt_bind_param($sql, 'ssiids', $id,$data['company'],$data['time'],$data['unit'],$data['cost'],$date);
		mysqli_stmt_execute($sql); mysqli_stmt_close($sql);
    }


    function Updater($db,$data) //Revisar que no se modifque ninguna FK!!!!!!!!!!!!!
    {
        $unit = 0 + $data['unit'];
        $cost = 0 + $data['cost'];
    	$sql = mysqli_prepare($db, "UPDATE upkeep SET time=?,unit=?,cost=? WHERE id = ?");
		mysqli_stmt_bind_param($sql, 'sids', $data["time"],$unit,$cost,$data["id"]);
		mysqli_stmt_execute($sql); mysqli_stmt_close($sql);
    }
